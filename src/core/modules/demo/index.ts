...
import { DemoMediatorCommand, DemoMediatorRequestDto, DemoMediatorResponseDto } from "./services/withMediatR";
...
export class DemoCommandHandler implements IRequestHandler<DemoCommand,DataResponse<DemoResponseDTO>>{
    ...
    protected async getMediatorResponse(value: DemoCommand): Promise<Result<DemoMediatorResponseDto, ResultError>>{

        const demoMediatorRequestDto=new DemoMediatorRequestDto();
        demoMediatorRequestDto.id=value?.request?.id;

        const mediatorResponse=await mediatR.send<DataResponse<DemoMediatorResponseDto>>(new DemoMediatorCommand(demoMediatorRequestDto));
    ...
    protected async getServiceResponse(value:string) : Promise<Result<string, ResultError>>{
        ...
        if(serviceResponse.isErr())
            return new Err(new ResultError(serviceResponse.error.message, serviceResponse.error.StatusCode));
    ...
    public async handle(value: DemoCommand): Promise<DataResponse<DemoResponseDTO>> {
        try
        {
            ...
            if(mediatorResponse.isErr())
                return DataResponseFactory.Response(false, mediatorResponse.error.StatusCode, null!, mediatorResponse.error.message);

            const serviceResponse=await this.getServiceResponse(value.request.value!);
            if(serviceResponse.isErr())
                return DataResponseFactory.Response(false, serviceResponse.error.StatusCode, null!, serviceResponse.error.message);

            const demoFunctionResponse=await demoFunction(value.request.value!);
            if(demoFunctionResponse.isErr())
                return DataResponseFactory.Response(false, demoFunctionResponse.error.StatusCode, null!, demoFunctionResponse.error.message);
