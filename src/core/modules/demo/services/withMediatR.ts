...
export class DemoMediatorCommandHandler
	implements IRequestHandler<DemoMediatorCommand, DataResponse<DemoMediatorResponseDto>>
{
	protected guard(value: DemoMediatorCommand): Result<unknown, ResultError> {
		if (!value) return new Err(new ResultError('Empty object', StatusCodes.BAD_REQUEST));
	...
	public async handle(
		value: DemoMediatorCommand
	): Promise<DataResponse<DemoMediatorResponseDto>> {
		try {
